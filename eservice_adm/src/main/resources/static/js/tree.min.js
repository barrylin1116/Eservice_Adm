...
(
	...
	function b(a, b, c) {
		...
		/* 勾選下層item時觸發上層folder勾選 *
		"item" === b.data("type") && a.options["selectable"] && checkItem(b) && b.parents(".tree-branch").eq(0).addClass("tree-selected") && b.parents(".tree-branch").eq(0).find(".tree-branch-header").find("i").eq(1).removeClass(a.options["unselected-icon"]).addClass(a.options["selected-icon"]),
		/* 勾選下層item時觸發最上層folder勾選 *
		"item" === b.data("type") && a.options["selectable"] && checkItem(b) && checkFolder(b.parents(".tree-branch").eq(1).find(".tree-branch")) && b.parents(".tree-branch").eq(1).addClass("tree-selected") && b.parents(".tree-branch").eq(1).find(".tree-branch-header").find("i").eq(1).removeClass(a.options["unselected-icon"]).addClass(a.options["selected-icon"]),
		/* 添加folder勾選置換圖片 */
		"folder" === b.data("type") && b.data("funcType") !== "func"  && a.options["selectable"] && c.hasClass(a.options["unselected-icon"]) && c.removeClass(a.options["unselected-icon"]).addClass(a.options["selected-icon"]),
		"folder" === b.data("type") && b.data("funcType") === "func" && c.eq(0).hasClass(a.options["unselected-icon"]) && c.removeClass(a.options["unselected-icon"]).addClass(a.options["selected-icon"]),
		/* folder被選定 */
		"folder" === b.data("type") && b.data("funcType") !== "func" && a.options["selectable"] && b.find(".tree-branch").addClass("tree-selected") && b.find(".tree-item").addClass("tree-selected")
		//"folder" === b.data("type") && "func" !== b.data("funcType") && a.options["selectable"] && b.find(".tree-branch").addClass("tree-selected")
		/* 點擊folder觸發上層folder勾選 *
		"folder" === b.data("type") && a.options["selectable"] && checkFolder(b) && b.parents(".tree-branch").eq(0).addClass("tree-selected") && b.parents(".tree-branch").eq(0).find(".tree-branch-header").find("i").eq(1).removeClass(a.options["unselected-icon"]).addClass(a.options["selected-icon"])
	*/}
	function c(a, b, c) {
		...
		/* 取消勾選下層item時觸發上層folder取消勾選 */
		//"item" === b.data("type") && a.options["selectable"] && b.parents(".tree-branch").removeClass("tree-selected") && b.parents(".tree-branch").eq(0).find(".tree-branch-header").find("i").eq(1).removeClass(a.options["selected-icon"]).addClass(a.options["unselected-icon"]),
		/* 取消勾選下層item時觸發最上層folder取消勾選 *
		"item" === b.data("type") && a.options["selectable"] && b.parents(".tree-branch").eq(1).find(".tree-branch-header").find("i").eq(1).removeClass(a.options["selected-icon"]).addClass(a.options["unselected-icon"]),
		/* 添加folder取消勾選置換圖片 */
		"folder" === b.data("type") && b.data("funcType") !== "func" && a.options["selectable"] && c.removeClass(a.options["selected-icon"]).addClass(a.options["unselected-icon"]),
		"folder" === b.data("type") && b.data("funcType") === "func" && c.eq(0).removeClass(a.options["selected-icon"]).addClass(a.options["unselected-icon"]),
		/* folder選定取消 */
		"folder" === b.data("type") && b.data("funcType") !== "func" && a.options["selectable"] && b.find(".tree-branch").removeClass("tree-selected") && b.find(".tree-item").removeClass("tree-selected")
		//"folder" === b.data("type") && a.options["selectable"] && b.find(".tree-branch").removeClass("tree-selected")
		/* 取消folder觸發上層folder取消勾選 *
		"folder" === b.data("type") && a.options["selectable"] && b.parents(".tree-branch").removeClass("tree-selected") && b.parents(".tree-branch").find(".tree-branch-header").find("i").eq(1).removeClass(a.options["selected-icon"]).addClass(a.options["unselected-icon"])
	*/}
	/* 檢查點擊Item時，所有上層的勾選狀態 */
	function checkItem(b) {
		var check = true;
		b.siblings(".tree-item").each(function (a, b){
			$(b).hasClass("tree-selected");
			if(!$(b).hasClass("tree-selected")){
				check = false;
			}
		});
		b.siblings(".tree-item").siblings(".tree-branch").each(function (a, c){
			$(c).hasClass("tree-selected");
			if(!$(c).hasClass("tree-selected")){
				check = false;
			}
		});
		return check;
	}
	/* 檢查點擊Folder時，所有上層的勾選狀態 */
	function checkFolder(b) {
		var check = true;
		b.each(function (a, b){
			$(b).hasClass("tree-selected");
			if(!$(b).hasClass("tree-selected")){
		...
		b.siblings(".tree-branch").each(function (a, b){
			$(b).hasClass("tree-selected");
			if(!$(b).hasClass("tree-selected")){
				check = false;
			}
	...
	g = function (b, c) {
		...
		this.options.folderSelect && (this.$element.addClass("tree-folder-select"), this.$element.off("click.fu.tree", ".tree-branch-name"),
			this.$element.on("click.fu.tree", ".icon-caret", a.proxy(function (b) {
					this.toggleFolder(a(b.currentTarget).next())
				}, this)),
			this.$element.on("click.fu.tree", ".tree-branch-header", a.proxy(function (b) {
	...
	g.prototype = {
		...
		selectTreeNode : function (b, c) {
			...
			g.dataForEvent = [],
			"folder" === c ? (f.$element = f.$element.closest(".tree-branch"), f.$icon = f.$element.find(".icon-item")) : f.$icon = f.$element.find(".icon-item"),
